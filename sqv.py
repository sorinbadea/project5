...
thread_running_ = 1
#
#"globalized" variables in the snmp thread
#@brief: maps containing pairs of stations and snmp values
#(lru default values and lru states)
snmp_lru_values_ = {}
snmp_lru_states_ = {}

class sqv_monitor:
    def __init__(self_, window_, tk_, stations_, devices_, snmp_):
        ...
        #@param devices_  in - devices table
        ...
        print(HEIGHT_OCCUPATION_RATIO * _screen_h)
    ...
    def display_header(self_, device_header_):
        ...
        label1_ = self_.tk_.Label(
            ...
            text="Stations", fg=TEXT_FG, font=(TEXT_FONT, STATION_TEXT_SIZE))
        label1_.grid(row=0, column=0, padx=1, pady=1, sticky="NSEW")
        label1_ = self_.tk_.Label(master = self_.window_)
        label1_.grid(row=0, column=1, padx=1, pady=1, sticky="NSEW")
        label1_ = self_.tk_.Label(master = self_.window_)
        label1_.grid(row=0, column=2, padx=1, pady=1, sticky="NSEW")
        # display the header
        # ------------------
        for (col_, dev_ ) in zip(self_.device_index_, device_header_):
            label_h_ = self_.tk_.Label(
                master = self_.window_,
                text=dev_, fg=TEXT_FG, font=(TEXT_FONT, HEADER_TEXT_SIZE))
            label_h_.grid(row=0, column=col_ + 3, padx=1, pady=1, sticky="NSWE")
        ...
        for (l_, col_, width_) in zip(self_.header_align_labels_, _columns, self_.cell_width_):
            l_.config(width=width_, height=0)
            l_.grid(row=1, column=col_, padx=1, pady=1, sticky="NSWE")

    def display_snmp_data(self_, snmp_lru_data_, snmp_lru_states_):
        # @brief display snmp data
        # @param in - snmp_lru_data_ table containing the lru default values
        # @param in - snmp_lru_states_ table containing the lru states values
        # @returns None
        lru_values_ = []
        lru_states_ = []
        ...
        for (l_, _col, _width) in zip(self_.scroll_align_labels_, _columns, self_.cell_width_):
            l_.config(width=_width, height=0)
            l_.grid(row=2, column=_col, padx=1, pady=0, sticky="NSWE")
        
        for (station_, lb_stations_, row_, display_row_, h_button_, sw_button_) in \
            zip(self_.stations_.keys(), self_.labels_stations_, rows_, display_rows_, self_.history_b_, self_.sw_b_):
            #get the snmp data for this station
            lru_values_ = snmp_lru_data_[station_]
            lru_states_ = snmp_lru_states_[station_]
            ...
            lb_stations_.config(
                text = text_l_, fg=text_color_, wraplength=STATION_IP_TEXT_SIZE * 10,
                font = (TEXT_FONT, STATION_IP_TEXT_SIZE))
            lb_stations_.grid(row=display_row_, column=0, padx=1, pady=3, sticky="NSEW")
            #display the history button
            #--------------------------
            h_button_.config(image = self_.history_image_)
            h_button_.grid(row=display_row_, column=1, padx=1, pady=0, sticky="NSEW")
            #display the sw button
            sw_button_.config(image = self_.info_image_)
            sw_button_.grid(row=display_row_, column=2, padx=1, pady=0, sticky="NSEW")
            # display the icon corresponding to each lru value and lru state
            #---------------------------------------------------------------
            columns_ = [i_ + OFFSET_ for i_ in range(len(lru_values_))]
            for (dev_, ldv_, ls_, col_) in zip(self_.devices_, lru_values_, lru_states_, columns_):
                #display lru default values
                self_.display_default_value(dev_, ldv_, row_, col_, display_row_, OFFSET_)
                #display lru states
                self_.display_lru_state(dev_, ls_, row_, col_, display_row_ + 1, OFFSET_)
        self_.frame_scrollable_.grid(
            row=3, column=0, columnspan = 14, rowspan = 10, padx=1, pady=0, sticky="NSEW")

    def display_lru_state(self_, device_id_, lru_state_, row_, col_, display_row_, OFFSET_):
        ...
        label_ = self_.labels_states_icons_[row_][col_ - OFFSET_] 
        image_ = (self_.icon_images_[icon2display_])
        label_.config(image = image_)
        label_.grid(row=display_row_, column=col_, padx=1, pady=3, sticky="NSEW")        

    def display_default_value(self_, device_id_, lru_value_, row_, col_, display_row_, OFFSET_):
        ...
        label_ = self_.labels_icons_[row_ ][col_ - OFFSET_] 
        image_ = (self_.icon_images_[icon2display_])
        label_.config(image = image_)
        label_.grid(row=display_row_, column=col_, padx=1, pady=0, sticky="NSEW")
    ...
    def display_about(self_):
        ...
        label1_ = tk.Label(master = new_window_, 
           ...
           fg='navy', font=(TEXT_FONT, HEADER_TEXT_SIZE))
        label1_.grid(row=0, column=0, padx=1, pady=3, sticky="NWSE")
        label2_ = tk.Label(master = new_window_, image=self_.viveris_)
...
def retrieve_snmp_data(stations_, snmp_, devices_, refresh_, checker_):
    ...
    global snmp_lru_values_
    global snmp_lru_states_
    while thread_running_:
        for s_ in stations_.keys():
            ...
            snmp_lru_values_[s_] = values_[snmp_tool.KEY_LRU_Values]
            snmp_lru_states_[s_] = values_[snmp_tool.KEY_LRU_States]
            checker_.check_history(values_, s_, devices_)
        time.sleep(refresh_)
...
if __name__ == "__main__":
    ...
    if args.config_file:
        ...
        if not parse_config_file(args.config_file, auth_data_, refresh_, stations_map_):
            sys.exit()
        #initialize the snmp values map
        for s_ in stations_map_.keys():
            snmp_lru_values_[s_] = []
            snmp_lru_states_[s_] = []
        ...
        #instanciate app
        sqv = sqv_monitor(window, tk, stations_map_, devices_, snmp_)
        ...
        sqv.display_header(grid_header_)
        ...
        thread_ = threading.Thread(
            ...
            args = (stations_map_, snmp_, grid_header_, refresh_[0], checker_))
        thread_.start()
        ...
        while thread_running_:
            sqv.display_snmp_data(snmp_lru_values_, snmp_lru_states_)
